  ...
  return sequelize.define(
    ...
    // 시퀄라이즈는 기본적으로 id를 기본키로 연결하므로 id 컬럼은 적을 필요가 없음
      ...
      defaultValue: sequelize.literal('now()'),
		...
		ah_time: {
			type: DataTypes.TIME, // VARCHAR -> STRING
		...
		ah_username: {
      type: DataTypes.STRING(40), // VARCHAR -> STRING
      allowNull: false, // NOT NULL -> allowNull
      unique: true, // UNIQUE -> unique
    },
		ah_mb_no: {
      ...
      unique: true, // UNIQUE -> unique
		...
		ah_agent_id: {
      type: DataTypes.ENUM(['user', 'tester', 'admin']), // VARCHAR -> STRING
      allowNull: false, // NOT NULL -> allowNull
      unique: true, // UNIQUE -> unique
    },
		ah_device: {
      type: DataTypes.ENUM(['normal', 'applicant', 'crew']), // VARCHAR -> STRING
      allowNull: false, // NOT NULL -> allowNull
      unique: true, // UNIQUE -> unique
    },
		ah_type: {
      type: DataTypes.ENUM(['true', 'false']), // VARCHAR -> STRING
      allowNull: false, // NOT NULL -> allowNull
      unique: true, // UNIQUE -> unique
    },
		ah_status: {
